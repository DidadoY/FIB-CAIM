...
if __name__ == '__main__':
    ...
    for i in range(args.iter):
        ...
        with mr_job1.make_runner() as runner1:
            ...
            for k in new_proto:
                prototypes += k + ":"
                for p in new_proto[k]:
                    prototypes += p[0] + "+" + str(p[1]) + " "
                prototypes += "\n"


            assignmentsPath = None
            protoPath = None
            
            if (i+1 == args.iter or nomove):
                assignmentsPath = "/assignments-final.txt"
                protoPath = "/prototypes-final.txt"
            else:
                assignmentsPath = "/assignments%d.txt"%(i+1)
                protoPath = "/prototypes%d.txt"%(i+1)
            
            outputAssign = open(cwd + assignmentsPath, 'w')
            outputAssign.write(docsAssigned)
            outputAssign.close()
            outputProtos = open(cwd + protoPath, 'w')
            outputProtos.write(prototypes)
            outputProtos.close()
